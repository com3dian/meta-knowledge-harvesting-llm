...
def encode_string_by_tiktoken(content: str, model_name: str = "gpt-4o"):
    global ENCODER
    if ENCODER is None:
        # For Gemini models, use cl100k_base encoding which is compatible
...
def save_knowledge_graph_to_pickle(nodes, edges, prefix="", write_result_to_txt=False):
    ...
    Args:
        nodes (dict): Dictionary containing the entities
        edges (dict): Dictionary containing the relationships
        write_result_to_txt (bool): Whether to also save as text file
    """
    ...
    if write_result_to_txt:
        entity_txt_file = f"outputs/{prefix}_result_{timestamp}_entities.txt"
        node_dict = dict(nodes)
        # Dump to YAML
        with open(entity_txt_file, 'w') as f:
            yaml.dump(node_dict, f, sort_keys=False, allow_unicode=True)
        
        link_txt_file = f"outputs/{prefix}_result_{timestamp}_links.txt"
        edge_dict = dict(edges)
        # Dump to YAML
        with open(link_txt_file, 'w') as f:
            yaml.dump(edge_dict, f, sort_keys=False, allow_unicode=True)
    
    return f"outputs/{prefix}_result_{timestamp}"

def read_output_file_as_string(file_path: str) -> str:
    """
    Read the contents of an output file as a string.
...
def read_knowledge_graph_from_pickle(file_path):
    """
    Read knowledge graph results from a pickle file.
...
def get_latest_result():
    """
    Get the most recent knowledge graph result.
    ...
    result_files = [f for f in os.listdir(output_dir) if f.endswith(".pkl")]
